...
{
	fprintf(out, "#ANN %s\n", ANNversion); // out << "#ANN " << ANNversion << "\n";
	// out.precision(ANNcoordPrec);		// use full precision in dumping
	if (with_pts) {						// print point coordinates
		fprintf(out, "points %d %d\n", dim, n_pts); // out << "points " << dim << " " << n_pts << "\n";
		for (int i = 0; i < n_pts; i++) {
			fprintf(out, "%d ", i); // out << i << " ";
...
void ANNkd_split::dump(					// dump a splitting node
		FILE *out)					// output stream
{
	fprintf(out, "split %d " FLOAT_OUT " " FLOAT_OUT " " FLOAT_OUT"\n", cut_dim, cut_val, cd_bnds[ANN_LO], cd_bnds[ANN_HI]);
	/*
		...
		FILE *out)					// output stream
{
	if (this == KD_TRIVIAL) {			// canonical trivial leaf node
		...
		fprintf(out, "leaf %d", n_pts); //  out << "leaf " << n_pts;
		for (int j = 0; j < n_pts; j++) {
			fprintf(out, " %d", bkt[j]); // out << " " << bkt[j];
		}
		fprintf(out, "\n"); // out << "\n";
	}
}
...
void ANNbd_shrink::dump(				// dump a shrinking node
		FILE *out)					// output stream
{
	fprintf(out, "shrink %d\n", n_bnds); // out << "shrink " << n_bnds << "\n";
	for (int j = 0; j < n_bnds; j++) {
		fprintf(out, "%d " FLOAT_OUT " %d\n", bnds[j].cd, bnds[j].cv, bnds[j].sd);
...
ANNkd_tree::ANNkd_tree(					// build from dump file
	FILE 		*in)					// input stream for dump file
{
...
ANNbd_tree::ANNbd_tree(					// build bd-tree from dump file
	FILE 		*in) : ANNkd_tree()		// input stream for dump file
{
	int the_dim;								// local dimension
	int the_n_pts;								// local number of points
	int the_bkt_size;							// local number of points
	...
	ANNpoint			&the_bnd_box_hi)		// high bounding point (ret'd)
{
	...
	//	Input file header
	//------------------------------------------------------------------
	fscanf(in, "%s", str); // in >> str;				// input header
	if (strncmp(str, "#ANN",STRING_LEN) != 0) {				// incorrect header
		annError((char*)"Incorrect header for dump file", ANNabort);
	}
	fgets(version, STRING_LEN, in); // in.getline(version, STRING_LEN);	// get version (ignore)
	...
	//			the dump file.
	//------------------------------------------------------------------
	fscanf(in, "%s", str); // in >> str;	// get major heading
	if (strcmp(str, "points") == 0) {			// points section
		fscanf(in, "%d", &the_dim); // in >> the_dim;			// input dimension
		fscanf(in, "%d", &the_n_pts); // in >> the_n_pts;		// number of points
								// allocate point storage
		//__android_log_print(ANDROID_LOG_DEBUG, HS_TAG, "Loading %d points of dimension %d.",
		//		the_n_pts, the_dim);
		the_pts = annAllocPts(the_n_pts, the_dim);
		for (int i = 0; i < the_n_pts; i++) {		// input point coordinates
			ANNidx idx;				// point index
			fscanf(in, "%d", &idx); // in >> idx;	// input point index
			if (idx < 0 || idx >= the_n_pts) {
				annError((char*)"Point index is out of range", ANNabort);
			}
			for (j = 0; j < the_dim; j++) {
				fscanf(in, FLOAT_SCAN, &the_pts[idx][j]); //in >> the_pts[idx][j];	// read point coordinates
			}
		}
	}
	else {							// no points were input
		annError((char*)"Points must be supplied in the dump file", ANNabort);
	}
	...
	//------------------------------------------------------------------
	fscanf(in, "%s", str); // in >> str;	// get next major heading
	if (strcmp(str, "tree") == 0) {				// tree section
		fscanf(in, "%d", &the_dim); // in >> the_dim;			// read dimension
		fscanf(in, "%d", &the_n_pts); // in >> the_n_pts;		// number of points
		fscanf(in, "%d", &the_bkt_size); // in >> the_bkt_size;	// bucket size
		...
		for (j = 0; j < the_dim; j++) {			// read bounding box low
			fscanf(in, FLOAT_SCAN, &the_bnd_box_lo[j]); // in >> the_bnd_box_lo[j];
		}
		for (j = 0; j < the_dim; j++) {			// read bounding box low
			fscanf(in, FLOAT_SCAN, &the_bnd_box_hi[j]); // in >> the_bnd_box_hi[j];
		}
		...
		if (next_idx != the_n_pts) {			// didn't see all the points?
			annError((char*)"Didn't see as many points as expected", ANNwarn);
		}
	}
	else {
		annError((char*)"Illegal dump format.	Expecting section heading", ANNabort);
	}
	return the_root;
}
	...
	int					&next_idx)				// next index (modified)
{
	...
	fscanf(in, "%s", tag); // in >> str;		// in >> tag;	// input node tag
	...
	if (strcmp(tag, "leaf") == 0) {				// leaf node
		fscanf(in, "%d", &n_pts); // in >> n_pts;	// input number of points
		...
		if (n_pts == 0) {						// trivial leaf
			...
			for (int i = 0; i < n_pts; i++) {	// input point indices
				fscanf(in, "%d", &the_pidx[next_idx++]); // in >> the_pidx[next_idx++];	// store in array of indices
			}
		}
	...
	//------------------------------------------------------------------
		...
		fscanf(in, "%d " FLOAT_SCAN " " FLOAT_SCAN " " FLOAT_SCAN, &cd, &cv, &lb, &hb); // in >> cd >> cv >> lb >> hb;
	...
	//------------------------------------------------------------------
		...
		if (tree_type != BD_TREE) {
			annError((char*)"Shrinking node not allowed in kd-tree", ANNabort);
		}

		fscanf(in, "%d", &n_bnds); // in >> n_bnds;	// number of bounding sides
												// allocate bounds array
		ANNorthHSArray bds = new ANNorthHalfSpace[n_bnds];
		for (int i = 0; i < n_bnds; i++) {
			fscanf(in, "%d " FLOAT_SCAN " %d", &cd, &cv, &sd); // in >> cd >> cv >> sd;	// input bounding halfspace
												// copy to array
		...
		annError((char*)"Illegal node type in dump file", ANNabort);
