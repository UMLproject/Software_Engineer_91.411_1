...
{
	for (int j = 0; j < dim; j++) {
		fprintf(out, FLOAT_OUT, pt[j]); //out << pt[j];
		if (j < dim-1) fprintf(out, " "); // out << " ";
	}
}
...
void annError(char *msg, ANNerr level)
{
	if (level == ANNabort) {
		...
		//		"ANN: ERROR------->%s<-------------ERROR", msg);
		// cerr << "ANN: ERROR------->" << msg << "<-------------ERROR\n";
		exit(1);
	}
	else {
		//__android_log_print(ANDROID_LOG_WARN, "HUMANSENSE",
		//		"ANN: WARNING------->%s<-------------WARNING", msg);
