	...
	ANNbool validate)					// true if average errors desired
{
	...
	__android_log_print(ANDROID_LOG_VERBOSE, "HUMANSENSE", "  (Performance stats:  [      mean :    stddev ]<      min ,       max >\n");
	// cout << "  (Performance stats: "
	//	  << " [      mean :    stddev ]<      min ,       max >\n";
	print_one_stat((char*)"    leaf_nodes       ", ann_visit_lfs, 1);
	print_one_stat((char*)"    splitting_nodes  ", ann_visit_spl, 1);
	print_one_stat((char*)"    shrinking_nodes  ", ann_visit_shr, 1);
	print_one_stat((char*)"    total_nodes      ", ann_visit_nds, 1);
	print_one_stat((char*)"    points_visited   ", ann_visit_pts, 1);
	print_one_stat((char*)"    coord_hits/pt    ", ann_coord_hts, ann_Ndata_pts);
	print_one_stat((char*)"    floating_ops_(K) ", ann_float_ops, 1000);
	if (validate) {
		print_one_stat((char*)"    average_error    ", ann_average_err, 1);
		print_one_stat((char*)"    rank_error       ", ann_rank_err, 1);
	}
	// cout.precision(0);					// restore the default
	__android_log_print(ANDROID_LOG_VERBOSE, "HUMANSENSE", "  )\n");
	// cout << "  )\n";
	// cout.flush();
}
