		...
		FILE *out)					// output stream
{
	...
	for (int i = 0; i < level; i++)				// print indentation
		fprintf(out, ".."); // out << "..";
	fprintf(out, "Shrink"); // out << "Shrink";
	for (int j = 0; j < n_bnds; j++) {			// print sides, 2 per line
		if (j % 2 == 0) {
			fprintf(out, "\n"); // out << "\n";						// newline and indentation
			for (int i = 0; i < level+2; i++) fprintf(out, "  "); //out << "  ";
		}
		fprintf(out, "  ([%d]%s" FLOAT_OUT ")", bnds[j].cd, (bnds[j].sd > 0 ? ">=" : "< "), bnds[j].cv);
		/*
		out << "  ([" << bnds[j].cd << "]"
			 << (bnds[j].sd > 0 ? ">=" : "< ")
			 << bnds[j].cv << ")";
	...
	ANNorthRect			&inner_box)		// inner box if shrinking (returned)
{
	...
	case ANN_BD_CENTROID:				// centroid shrink
		...
		break;
	default:
		annError((char*)"Illegal shrinking rule", ANNabort);
	}
	return decomp;
}
