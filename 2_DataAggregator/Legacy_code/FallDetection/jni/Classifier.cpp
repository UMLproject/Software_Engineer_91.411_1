...
void Classifier::classifyAndSave(ANNcoord** data, long length, FILE *fout) {
	if (models == NULL) {
		return;
	}
...
CvMat* Classifier::classify(ANNcoord** data, long length) {
	if (models == NULL) {
		return NULL;
	}
	...
	//Dislikes the array not having a constant value
		...
		for (k = 0; k < M; k++) {
			...
			if (algorithm == 1) {
				...
				for (j = 0; j < matchSteps; j++) {
					...
					if (l < numNeighbours) {
						//HS_2("Couldn't find enough neighbours (found: %d, required: %d).", l, numNeighbours);
					}
...
//									"Bad neighbour %d.", nn_idx[l]);
							...
							if (a >= extra_neighbours) {
								//HS_("Couldn't find enough good neighbours.");
								nn_idx[l] = 0;
								break;
							}
						}
