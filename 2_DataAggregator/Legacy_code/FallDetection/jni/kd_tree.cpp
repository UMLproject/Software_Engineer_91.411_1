...
#include "ANN/kd_split.h"					// kd-tree splitting rules
		...
		FILE	*out)					// output stream
{
	...
	for (int i = 0; i < level; i++)		// print indentation
		fprintf(out, ".."); // out << "..";
	fprintf(out, "Split cd=%d cv=" FLOAT_OUT " lbnd=" FLOAT_OUT " hbnd=" FLOAT_OUT "\n", cut_dim, cut_val, cd_bnds[ANN_LO], cd_bnds[ANN_HI]);
	/* PORTED
		...
		FILE	*out)					// output stream
{
	...
	if (this == KD_TRIVIAL) {			// canonical trivial leaf node
		fprintf(out, "Leaf (trivial)\n"); // out << "Leaf (trivial)\n";
	}
	else{
		fprintf(out, "Leaf n=%d <", n_pts); // out << "Leaf n=" << n_pts << " <";
		for (int j = 0; j < n_pts; j++) {
			fprintf(out, FLOAT_OUT, bkt[j]); // out << bkt[j];
			if (j < n_pts-1) fprintf(out, ","); // out << ",";
		}
		fprintf(out, ">\n"); // out << ">\n";
	}
}
...
void ANNkd_tree::Print(					// print entire tree
		...
		FILE	*out)					// output stream
{
	fprintf(out, "ANN Version %s\n", ANNversion); // out << "ANN Version " << ANNversion << "\n";
...
ANNkd_tree::ANNkd_tree(					// construct from point array
	...
	int					dd,				// dimension
	int					bs,				// bucket size
	ANNsplitRule		split = ANN_KD_STD)			// splitting method
{
